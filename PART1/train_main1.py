...
data_path = os.listdir(r"F:\pywork119\pythonProject\segmentation\PART1\test1")
data_path.sort(key=lambda x: int(x[:-4]))
data_path1 = os.listdir(r"F:\pywork119\pythonProject\segmentation\PART1\label1")
...
def default_loader(path):
    data_pil = np.load(r"F:\pywork119\pythonProject\segmentation\PART1\test1/%s" % (path)).reshape((1, 256, 256))
    data_tensor = torch.tensor(data_pil).type(torch.FloatTensor)
    return data_tensor

def default_loader1(path):
    data_pil1 = np.load(r"F:\pywork119\pythonProject\segmentation\PART1\label1/%s" % (path)).reshape((1, 256, 256))
...
for epoch in range(epochs):
    ...
    if epoch % 100 == 0 and epoch != 0:
        torch.save(net.state_dict(), f"DU1/net_{epoch}.pth")


torch.save(net.state_dict(), "DU1/net.pth")


np.savetxt("DU1/Train Loss.csv", np.array(Loss_list))
np.savetxt("DU1/Val Loss.csv", np.array(Val_loss_list))
